...
<?php
    ...
    $query = mysqli_query($result,"select shortname,fullname,summary from mdl_course where id = 7");
...
<?php
    ...
    $result = mysqli_connect("localhost","root","","moodle") or die("Could not connect to database." .mysqli_error());
    mysqli_select_db($result, "moodle") or die("Could not select the databse." .mysqli_error());
    $query = mysqli_query($result,"select shortname,fullname,summary from mdl_course where id = 8");
    while($rows = mysqli_fetch_array($query))
    {
        $sname = $rows['shortname'];
        $fname = $rows['fullname'];
...
<?php
    ...
    $result = mysqli_connect("localhost","root","","moodle") or die("Could not connect to database." .mysqli_error());
    mysqli_select_db($result, "moodle") or die("Could not select the databse." .mysqli_error());
    $query = mysqli_query($result,"select shortname,fullname,summary from mdl_course where id = 9");
    while($rows = mysqli_fetch_array($query))
    {
        $sname = $rows['shortname'];
        $fname = $rows['fullname'];
